...
輸出格式要求：
{
  ...
  "options": {
    ...
    "toppings": [{"name": "珍珠", "price": 10}, {"name": "椰果", "price": 10}]
  }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!parsedMenu.options) {
      parsedMenu.options = { sugar: [], ice: [], toppings: [] };
    }
    
    // Normalize singular to plural
    if (parsedMenu.options.topping && !parsedMenu.options.toppings) {
      parsedMenu.options.toppings = parsedMenu.options.topping;
    }

    // Default missing properties to empty arrays to prevent frontend crashes
    parsedMenu.options.sugar = parsedMenu.options.sugar || ["無糖", "微糖", "半糖", "少糖", "正常甜"];
    parsedMenu.options.ice = parsedMenu.options.ice || ["正常冰", "少冰", "微冰", "去冰", "溫", "熱"];
    parsedMenu.options.toppings = parsedMenu.options.toppings || [];
